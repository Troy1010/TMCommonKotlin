package com.tminus1010.tmcommonkotlin.rx3.transformers
 ...
 */
fun VolumeTransformer() = ObservableTransformer<ByteArray, Double> { upstream ->
    upstream
        // # Regroup samples into a new time interval, and cast bytes to ints.
        .concatMapIterable { it.asIterable() }
        .map { it.toInt() }
        ...
        .publish { upstreamAverages ->
            ...
            { currentValue: Double, min: Double, max: Double ->
                // ## When a sufficient margin is achieved, lock min and max after a delay.
