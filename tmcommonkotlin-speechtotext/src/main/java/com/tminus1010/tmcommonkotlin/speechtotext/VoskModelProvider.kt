...
sealed class VoskModelProvider {
    abstract val model: Single<Model>

    class Any(private val application: Application) : VoskModelProvider() {
        override val model: Single<Model> = Internal(application).model.onErrorResumeNext { External(application).model }
    }

    class Internal(private val application: Application) : VoskModelProvider() {
        override val model: Single<Model> =
            Single.create { downstream ->
     ...
     */
    class External(private val application: Application) : VoskModelProvider() {
