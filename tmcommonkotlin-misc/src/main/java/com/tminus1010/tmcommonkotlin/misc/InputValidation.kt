    ...
    companion object {
        val asCardNumber = { cardNumber: String ->
            if (cardNumber.isEmpty())
                Result.Error(Exception("Required"))
            else if (Regex("""[A-z]""").containsMatchIn(cardNumber))
                Result.Error(Exception("Must not contain letters"))
            else if (Regex("""[0-9]""").findAll(cardNumber).count() != 16)
                Result.Error(Exception("Must contain 16 digits"))
            else
        ...
        val asStreetAddress = { streetAddress: String ->
            if (streetAddress.isEmpty())
                ...
                Result.Error(Exception("Must have a number"))
            else if (!Regex("""[0-9][0-9]*\s..*""").containsMatchIn(streetAddress.trim()))
                Result.Error(Exception("Must have a street name"))
            else {
        ...
        val asAptNum = { aptNum: String ->
            if (Regex(""".\s.""").containsMatchIn(aptNum))
                Result.Error(Exception("Must not contain spaces"))
            else
        ...
        val asState = { state: String ->
            if (state.isEmpty())
                ...
                Result.Error(Exception("Must contain two letters"))
            else
        ...
        val asEmail = { email: String ->
            if (email.isEmpty())
                ...
                Result.Error(Exception("Must contain an email domain"))
            else if (!Regex("""..*@..*\.com\s*${'$'}""").containsMatchIn(email))
                Result.Error(Exception("Must contain an email name"))
            else
        ...
        val asPassword = { s: String ->
            if (s.isEmpty())
                ...
                Result.Error(Exception("Must have at least 6 characters"))
            else if (!Regex("""[0-9]""").containsMatchIn(s))
                Result.Error(Exception("Must contain at least 1 digit"))
        ...
        val asPhone = { phone: String ->
            if (phone.isEmpty())
                ...
                Result.Error(Exception("Must only contain digits"))
            else if (phone.length != 10)
                Result.Error(Exception("Must have 10 characters"))
            else
